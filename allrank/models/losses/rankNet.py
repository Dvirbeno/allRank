...
def directedRankNet(y_pred, y_true, padded_value_indicator=PADDED_Y_VALUE, weight_by_diff=False,
    ...
    # we can do that since in the candidate pairs we had symetric pairs so we can stick with
    # positive ones for a simpler loss function formulation
    the_mask = (pred_diffs >= 0) & (~torch.isinf(true_diffs))
...
def marginRankNet(y_pred, y_true, padded_value_indicator=PADDED_Y_VALUE):
    """
    RankNet loss introduced in "Learning to Rank using Gradient Descent".
    :param y_pred: predictions from the model, shape [batch_size, slate_length]
    :param y_true: ground truth labels, shape [batch_size, slate_length]
    ...
    # here we calculate the relative true relevance of every candidate pair
    true_diffs = pairs_true[:, :, 0] - pairs_true[:, :, 1]
    pred_diffs_high, pred_diffs_low = selected_pred[:, :, 0], selected_pred[:, :, 1]
    ...
    pred_diffs_high, pred_diffs_low = pred_diffs_high[the_mask], pred_diffs_low[the_mask]
    ...
    return MarginRankingLoss()(pred_diffs_high, pred_diffs_low, true_diffs)
